"""A-SYNC no-perturb 8-cycle on Qwen2.5-7B (28L).

Key finding on 0.5B: perturbation causes oscillations (7.8 -> 23.9 bounce).
Without perturbation, A-SYNC is monotonic (9.1 -> 5.6). 
This tests the same on 7B — the largest model, deepest boundary.
"""
...
print(f"\nA-SYNC NO-PERTURB 8-CYCLE on Qwen2.5-7B (28L)")
print("Config: sync=0.05, decay=0.8, lr=2e-4, NO perturbation")
...
result = {
    ...
    "sync_strength": 0.05, "sync_decay": 0.8,
    "final_ppl": ppls[-1] if ppls else None,
    "best_ppl": min(ppls) if ppls else None,
    "perturb": False,
}
print(f"\nA-SYNC no-perturb 8-cycle: {elapsed:.0f}s")
print(f"  PPL: {' -> '.join(f'{x:.1f}' for x in ppls)}")
print(f"  Best: {min(ppls):.1f}, Final: {ppls[-1]:.1f}")

with open("runs/a_sync_noperturb_8cycle_7b.json", "w") as f:
    json.dump(result, f, indent=2, default=str)
print("Saved runs/a_sync_noperturb_8cycle_7b.json")
