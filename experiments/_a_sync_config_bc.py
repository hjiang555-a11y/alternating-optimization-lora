"""A-SYNC configs B and C on Qwen2.5-7B (28L).

Config B: sync_strength=0.02, decay=0.95 (weak sync, slow decay)
Config C: sync_strength=0.01, decay=1.0  (minimal sync, constant)
"""
...
def run_one(sync_strength, sync_decay, name):
    print(f"\n{'='*50}\nA-SYNC {name}\n{'='*50}")
...
# ── Main ──
results = {}
results["B_0.02_d0.95"] = run_one(0.02, 0.95, "Config B: sync=0.02, decay=0.95")
results["C_0.01_d1.0"] = run_one(0.01, 1.0, "Config C: sync=0.01, decay=1.0")

print(f"\n{'='*60}")
print("Qwen2.5-7B A-SYNC Config Sweep")
for label, r in results.items():
    p = r["ppls"]
    pstr = " -> ".join(f"{x:.1f}" if x < 1e10 else "inf" for x in p)
    print(f"  {label}: {pstr}  ({r['elapsed']:.0f}s)")

# Merge with existing config A
try:
    prev = json.load(open("runs/a_sync_qwen7b.json"))
except: prev = {"results": {}}
