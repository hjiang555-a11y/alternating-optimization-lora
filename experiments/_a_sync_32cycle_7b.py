"""A-SYNC 32-cycle on Qwen2.5-7B (28L) — find convergence asymptote.

At 16 cycles A-SYNC reaches PPL 10.5 and is still dropping. 32 cycles
will establish the floor — whether convergence plateaus or continues.

Config: sync=0.05, cosine decay, no perturbation, lr=2e-4, momentum=0.
"""
...
print(f"\nA-SYNC 32-CYCLE on Qwen2.5-7B (28L)")
print("Config: sync=0.05, cosine decay, no perturb, lr=2e-4, momentum=0")
...
for cyc in range(N_CYCLES):
    ...
    print(f"  C{cyc+1:2d}: ppl={ppl_str}, sync={sync_strength:.4f}, lr={lr:.6f}")
    if ppl > 1e10: break
...
result = {
    "ppls": ppls, "elapsed": elapsed, "n_cycles": N_CYCLES,
    "sync_strength_0": 0.05, "decay": "cosine",
    "final_ppl": ppls[-1], "best_ppl": min(ppls),
}
print(f"\nA-SYNC 32-cycle: {elapsed:.0f}s ({elapsed/3600:.1f}h)")
print(f"  PPL: {' -> '.join(f'{x:.1f}' for x in ppls)}")
print(f"  Best: {min(ppls):.1f}, Final: {ppls[-1]:.1f}")

with open("runs/a_sync_32cycle_7b.json", "w") as f:
    json.dump(result, f, indent=2, default=str)
print("Saved runs/a_sync_32cycle_7b.json")
