...
**Status**: Final Draft v3.4 — 17 experiments complete (P0-P5 + F1-F2 + A + E4 + Critical r=4 + E2 + X1-X3); Grok review passed (Minor Revision → Accept)  
**Date**: 2026-06-22  
**Previous**: v3.3 (2026-06-23)
...
3. **Full-rank underperforms LoRA on this task** — even r=8 (1.1M params) beats full-rank (494M params) by 27× on Qwen2.5-0.5B WikiText-2. On 7B, r=64 (40M) achieves PPL=1.41 vs full-rank PPL=1.25 — 175× parameter efficiency for 1.13× PPL difference.
...
with $\eta \approx 230$ for WikiText-2 post-training. This is the minimal LoRA rank at which post-training perplexity saturates. For $r \geq r_{\min}$, PPL is independent of $r$. For $r < r_{\min}$, PPL degrades. For currently popular architectures ($L/d_h \leq 0.035$), $r_{\min} \leq 8$ — r=8 is sufficient for the WikiText-2 task on these models.
...
The paper's longest-running open question — what determines $\eta$ — has been resolved through systematic candidate elimination. We tested three mechanisms:

1. **Token entropy scaling.** Falsified (P0): r=8 plateau in both Chinese and English.
2. **Training budget scaling.** Falsified (F1): r4/r8 ≈ 1.006 constant across N=400/800/1600.
3. **Pretraining quality modulation.** CONFIRMED: SmolLM2-135M r=4 → PPL=88.22 (50× vs plateau) but Qwen2.5-0.5B r=4 → PPL=1.63 (at plateau). SmolLM2 (2T tokens) needs r_min≈12; Qwen (18T tokens) needs only r_min≈4. Pretraining quality affects per-layer representation quality, which modulates the LoRA correction required.

$\eta$ is NOT a universal constant — it depends on pretraining quality × L/d_h. This completes the mechanism search (§6.9.3): r=8 is at plateau in both languages (r8/r32=1.02 in Chinese vs 1.01 in English). **The mechanism determining $\eta$ remains an open question.** We tested two candidate mechanisms: task intrinsic dimensionality (predicting $\eta$ constant across training budgets) and training budget scaling (predicting $\eta \propto 1/N_{\text{samples}}$). A r=4 vs r=8 comparison at $N_{\text{samples}} = 400, 800, 1600$ on Qwen2.5-0.5B (AdamW, 100 steps, seed 42) yielded r4/r8 ratios of 1.005, 1.006, and 1.008 — all at plateau. **$\eta$ is independent of training budget.** Combined with the language-independence finding (§6.9.3), the converging evidence supports $\eta$ as an empirically stable architectural constant under tested conditions — determined by the interaction between the post-training objective and the residual stream structure, independent of both surface token statistics and data quantity. The precise mechanism (task intrinsic dimensionality or other) remains to be identified, but two parsimonious alternatives have been eliminated.
...
2. **Full-rank fine-tuning catastrophically overfits on small data**, producing near-perfect in-distribution perplexity (PPL=1.25 at 7B, 106× over baseline) while *reducing* downstream accuracy versus the untrained model (HellaSwag: −3.2pp, MMLU: −4.2pp, ARC: −3.3pp). LoRA r=8 preserves >99% of baseline accuracy on all three tasks while using 0.04% of the parameters. The M-index ($M < 1$ when $N_p/N_d > 10^4$) is a lightweight diagnostic that reliably flags this memorization regime, into which full-rank always falls at practical post-training data budgets.
